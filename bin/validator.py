"""
This file contains the classes that can be used for the validation 
of the parameters given to the Custom commands by user.
"""
import re
        ...
        if value is not None:
            if not isinstance(value, bool):
                ...
                if value not in Boolean.truth_values:
                    raise ValueError('Please specify boolean value for parameter: {0}, given value: {1}'.format(self.option_name, value))
    ...
    def validate(self, value):
        ...
        if value is not None:
            ...
            if self.pattern.match(value) is None:
                raise ValueError('Given value: \'{0}\' for parameter: {1} does not match the valid Splunk field pattern'.format(value, self.option_name))
        return value
        ...
        if minimum is not None and maximum is not None:
            def check_range(value):
                if not (minimum <= value <= maximum):
                    raise ValueError('Value of parameter {0} should be between {1} and {2}, given value: {3}'.format(self.option_name, minimum, maximum, value))
                return
        elif minimum is not None:
            def check_range(value):
                if value < minimum:
                    raise ValueError('Value of parameter {0} should be greater than {1}, given value: {2}'.format(self.option_name, minimum, value))
                return
        elif maximum is not None:
            def check_range(value):
                if value > maximum:
                    raise ValueError('Value of parameter {0} should be lesser than {1}, given value: {2}'.format(self.option_name, maximum, value))
                return
        else:
            def check_range(value):
                return
    ...
    def validate(self, value):
        ...
        if value is not None:
            try:
                ...
                raise ValueError('Please specify integer value for parameter: {0}, given value: {1}'.format(self.option_name, value))
