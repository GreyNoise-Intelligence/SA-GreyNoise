...
def _filter_content(content, *args):
    ...
    return record((k, v) for k, v in content.items() if k not in ["eai:acl", "eai:attributes", "type"])
...
class Entity(Endpoint):
    ...
    def _proper_namespace(self, owner=None, app=None, sharing=None):
        ...
        if owner is None and app is None and sharing is None:  # No namespace provided
            ...
            return (self.service.namespace["owner"], self.service.namespace["app"], self.service.namespace["sharing"])
    ...
    def read(self, response):
        ...
        unquoted_links = dict((k, UrlEncoded(v, skip_encode=True)) for k, v in results["links"].items())
    ...
    def __getitem__(self, key):
        ...
        **Example**::
        ...
        try:
            ...
            if len(entries) > 1:
                raise AmbiguousReferenceException(f"Found multiple entities named '{key}'; please specify a namespace.")
            if len(entries) == 0:
                raise KeyError(key)
            return entries[0]
    ...
    def clean(self, timeout=60):
        ...
        try:
            ...
            if self.content.totalEventCount != "0":
                raise OperationError(f"Cleaning index {self.name} took longer than {timeout} seconds; timing out.")
        finally:
...
class Inputs(Collection):
    ...
    def create(self, name, kind, **kwargs):
        ...
        name = UrlEncoded(name, encode_slash=True)
        path = _path(self.path + kindpath, f"{kwargs['restrictToHost']}:{name}" if "restrictToHost" in kwargs else name)
