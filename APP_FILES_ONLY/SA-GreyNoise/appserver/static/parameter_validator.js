...
require([
    ...
    function process_inputs(){
        ...
        for (i = 0; i < requiredFields.length; i++) {
            var raw = tokens.get(requiredFields[i]);
            if (!re.test(String(raw == null ? "" : raw).trim())) {
                is_invalid = 0;
                break;
            }
        }
        ...
        for (i = 0; i < tokensToEscapeSpecialCharacter.length; i++) {
            var rawTok = tokens.get(tokensToEscapeSpecialCharacter[i]);
            let tokenVal = String(rawTok == null ? "" : rawTok).trim();
