    ...
    def parse(cls, command, argv):
        ...
        for option in cls._options_re.finditer(command_args.group("options")):
            ...
            if name not in command.options:
                raise ValueError(f"Unrecognized {command.name} command option: {name}={json_encode_string(value)}")
        ...
        if missing is not None:
            if len(missing) > 1:
                raise ValueError(f'Values for these {command.name} command options are required: {", ".join(missing)}')
...
class ConfigurationSettingsType(type):
    ...
    specification_matrix = {
        ...
        "streaming_preop": specification(type=(bytes, str), constraint=None, supporting_protocols=[1, 2]),
        "type": specification(
            type=(bytes, str),
