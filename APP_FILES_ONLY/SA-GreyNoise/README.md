...
This app contains the following dashboards:
 ...
 - GreyNoise Executive Dashboard: Firewall and VPN traffic enriched with GreyNoise feed verdicts, including executive summary and SOC triage views. Requires a configured Feed import and supported firewall or VPN logs.
...
*Note : After upgrading from a version that predates Business Service Intelligence, Overview percentages and Queried IP Addresses BSI status apply only to IPs scanned after the upgrade (or after running the Scan Deployment lookup migration saved search).*

# SAVED SEARCHES #
This app contains the following saved searches:

 - greynoise_scan_deployment_once: Populates `gn_scan_deployment_ip_lookup` and is triggered after configuring Scan Deployment. Scans the previous 24 hours of indexed data.
 - greynoise_scan_deployment: Populates `gn_scan_deployment_ip_lookup` hourly (`0 * * * *`) and scans the previous 70 minutes of indexed data.
 ...
 - greynoise_feed_once: Populates `greynoise_indicators` with `gnfeed` on demand when enabling a Feed (or forcing a run).
 ...
 - greynoise_callback_feed_once: Populates `greynoise_callback_indicators` with `gncallbackfeed` on demand when enabling Callback IP Feed (or forcing a run).
...
*Note : greynoise_scan_deployment_once and greynoise_scan_deployment scan data already indexed in Splunk. If a run is skipped, IPs indexed during that interval are not added to `gn_scan_deployment_ip_lookup` until a later scan covers them. Feed, Callback feed, purge, and ES intel populate searches are disabled until the corresponding feature is enabled in Configuration.*
