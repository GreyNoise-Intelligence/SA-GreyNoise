...
if hasattr(typing, "ClassVar"):
    ...
    class _ClassVarMeta(typing.TypingMeta):
        ...
        def __getitem__(self, item):
            ...
            param = typing._type_check(item, "{} accepts only single type.".format(cls.__name__[1:]))
        ...
        def _eval_type(self, globalns, localns):
            ...
            if new_tp == self.__type__:
                return self
            return type(self)(self.__name__, self.__bases__, dict(self.__dict__), tp=self.__type__, _root=True)
...
if hasattr(typing, "ContextManager"):
    ...
    class ContextManager(
        typing.Generic[T_co], metaclass=_ExtensionsGenericMeta, extra=contextlib.AbstractContextManager
    ):
        __slots__ = ()
...
if hasattr(typing, "AsyncContextManager"):
    ...
    class AsyncContextManager(
        typing.Generic[T_co], metaclass=_ExtensionsGenericMeta, extra=contextlib.AbstractAsyncContextManager
    ):
        __slots__ = ()
    ...
    class _ProtocolMeta(GenericMeta):
        ...
        if not OLD_GENERICS:
            ...
            def __new__(cls, name, bases, namespace, tvars=None, args=None, origin=None, extra=None, orig_bases=None):
                ...
                self.__args__ = (
                    tuple(... if a is _TypingEllipsis else () if a is _TypingEmpty else a for a in args)
                    if args
                    else None
                )
            ...
            if "__orig_bases__" in cls.__dict__:
                ...
                if gvars is None:
                    ...
                    if not tvarset <= gvarset:
                        ...
                        raise TypeError(
                            "Some type variables ({}) are" " not listed in {}[{}]".format(s_vars, the_base, s_args)
                        )
                    tvars = gvars
...
if sys.version_info[:2] >= (3, 10):
    ...
    def get_origin(tp):
        ...
        if isinstance(tp, _AnnotatedAlias):
            return Annotated
        if isinstance(tp, (_GenericAlias, GenericAlias, _BaseGenericAlias, ParamSpecArgs, ParamSpecKwargs)):
...
class _ConcatenateGenericAlias(list):
    ...
    def __repr__(self):
        ...
        return "{origin}[{args}]".format(
            origin=_type_repr(self.__origin__), args=", ".join(_type_repr(arg) for arg in self.__args__)
        )
...
if hasattr(typing, "TypeGuard"):
    ...
    class _TypeGuard(typing._FinalTypingBase, _root=True):
        ...
        def __getitem__(self, item):
            ...
            if self.__type__ is None:
                return cls(
                    typing._type_check(item, "{} accepts only a single type.".format(cls.__name__[1:])), _root=True
                )
            raise TypeError("{} cannot be further subscripted".format(cls.__name__[1:]))
