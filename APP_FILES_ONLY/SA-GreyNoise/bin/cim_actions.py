...
def default_dropexp(x):
    ...
    """
    return (
        (x.startswith("_") and x not in ["_bkt", "_cd", "_raw", "_time"])  # noqa: W503
        or x.startswith("date_")  # noqa: W503
        or x in ["punct", "sid", "rid", "orig_sid", "orig_rid"]  # noqa: W503
    )
...
class ModularAction(object):
        ...
        if self.sid and not self.job and not self.replay_mode:
            try:
                r, c = rest.simpleRequest(
                    "search/jobs/%s" % self.sid, sessionKey=self.session_key, getargs={"output_mode": "json"}
                )
    ...
    def message(self, signature, status=None, rids=None, level=logging.INFO, **kwargs):
        ...
        newargs = [
            x for x in sorted(kwargs) if (x not in ModularAction.DEFAULT_MSGFIELDS) and re.match(r"[A-Za-z_]+", x)
        ]
        # MSG
        msg = "{0} {1}".format(
            ModularAction.DEFAULT_MESSAGE, " ".join(['{i}="{{d[{i}]}}"'.format(i=i) for i in newargs])
        )
    ...
    def result2stash(self, result, dropexp=default_dropexp, mapexp=default_mapexp, addinfo=False):
        ...
        for key in sorted(list(result_keys)):
            ...
            if (
                mv_key in result
                and isinstance(result[mv_key], six.string_types)  # noqa: W503 string
                and result[mv_key].startswith("$")  # noqa: W503 prefix
                and result[mv_key].endswith("$")  # noqa: W503
            ):  # suffix
    ...
    def writeevents(self, index="summary", host=None, source=None, fext="common_action_model"):
        ...
        if self.events:
            ...
            for chunk in (
                self.events[x : x + self.DEFAULT_CHUNK] for x in range(0, len(self.events), self.DEFAULT_CHUNK)
            ):
                ...
                # write output string
                try:
                    fn = "{0}_{1}.stash_{2}".format(mktimegm(time.gmtime()), random.randint(0, 100000), fext)
    ...
    def queuework(self, exit_strategy=None, tries=3, raise_all=False):
        ...
        for unused_x in range(0, tries):
            try:
                r, c = rest.simpleRequest(self.CAM_QUEUE_URI, sessionKey=self.session_key, jsonargs=json.dumps(records))
                c = c.decode("utf-8")
...
class ModularActionTimer(object):
    ...
    def __exit__(self, *args):
        try:
            ...
            self.modaction.message("Modular action script duration", component=self.component, duration=self.interval)
        except Exception:
